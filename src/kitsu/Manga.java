...
ImageIcon icon1 = new ImageIcon("C:\\Users\\devil\\Desktop\\Kitsu (1)\\Kitsu\\src\\kitsu\\Images\\"+home.anime+".jpg");
    ...
    public Manga() {
        ...
        String score = "", published, summary, anime_type = "", volumes, chapters;
        float avg = 0.0f;
        int div = 1;
        String temp;
        boolean check = true;
        ...
        System.out.println("before try"); // try catch to get the anime name from db
        try{
            ...
            if (anime_type.equals("Manga")){
                ...
                while(result.next()){
                    ...
                    published = result.getString(9);
                    anime_type = result.getString(8);
                    chapters = result.getString(10);
                    volumes = result.getString(11);
                }
    ...
    public void table1(){
        ...
        try{
            java.sql.Connection mycon = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","hr","hr");//Nom	hr@//localhost:1521/xe
            Statement stmt = mycon.createStatement();
            ResultSet rs = stmt.executeQuery("Select * from anime natural join manga where name ='"+home.anime+"'");
            ...
            while(rs.next()){
                ...
                for(int i = 1; i <= c; i++){
                    v2.add(rs.getString("publishdate"));
                    v2.add(rs.getString("chapters"));
                    v2.add(rs.getString("volumes"));
                }
                df.addRow(v2);
            }
        }
